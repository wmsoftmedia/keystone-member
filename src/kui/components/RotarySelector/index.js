...
class RotarySelector extends React.Component {
  ...
  calcAngle = value => {
    return this.props.endValue ? -(2 * Math.PI * value) / this.props.endValue : 0
  }
  ...
  limitAngle = value => {
    ...
    if (minValue && maxValue) {
      ...
      return absValue > minAngle ? (absValue < maxAngle ? value : maxAngle) : minAngle
    } else {
      return value
    }
  }
    ...
    this.onTiltGestureEvent = Animated.event(
      [{ nativeEvent: { x: this._pointX, y: this._pointY } }],
      {
        listener: event => {
          ...
          const rotateAngle = this.limitAngle(this._rotateAngle._value + angleDiff)
  ...
  render() {
    ...
    const circleSize = size - markerSize / Math.SQRT2 - (markerSize / 2 - borderWidth / 2)

    return (
      <View width={size} height={size} opacity={this.props.disabled ? 0.5 : 1}>
        <PanGestureHandler
          ref={this.panRef}
          onGestureEvent={this.props.disabled ? undefined : this.onTiltGestureEvent}
          ...
          avgTouches
        >
          <View style={styles.container} collapsable={false} width={size} height={size}>
            <Animated.View
              style={{
