...
import { FORM_NAME } from "./MealScreen"
...
const readState = state => (key, defaultVal) =>
  getOr(defaultVal, `app.kitchenMeal.${key}`, state)
...
const enhance = connect(
  state => {
    ...
    const items = getOr([], `${FORM_NAME}.items`, state)
    return {
      isButtonAvailable: !isLoading && !isError && !!name && items.length > 0
    }
  },
  dispatch => ({
