...
import { FORM_NAME } from "scenes/Home/Nutrition/Kitchen/NewMeal/MealScreen"
import {
  dataToFood,
  SOURCE_CACHE_FS,
  SOURCE_FS_FULL,
...
const addFood = (client, food) => async (dispatch, getState) => {
  ...
  try {
    ...
    const loadedFood = getOr([], `${FORM_NAME}.items`, getState())
    // update required meal
    dispatch(
      actions.change(`${FORM_NAME}.items`, [
        ...loadedFood,
    ...
    logErrorWithMemberId(memberId => {
      Sentry.captureException(
        new Error(
          `MId:{${memberId}}, Scope:{Nutrition.Kitchen.NewMeal.actions.addItems}, ${_.toString(
            e
          )}`
        )
      )
